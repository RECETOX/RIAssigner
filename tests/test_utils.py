...
@pytest.mark.parametrize(
    ...
    [
        [["blub", "test", "peter"], ["a", "b", "peter", "d"], "peter"],
        [["a"], ["b"], None],
    ],
)
...
@pytest.mark.parametrize(
    "filename, expected", [("test_file.csv", ","), ("test_file.tsv", "\t")]
)
...
@pytest.mark.parametrize(
    "filename, expected", [("test_file.csv", "csv"), ("test_file.tsv", "tsv")]
)
...
@pytest.mark.parametrize(
    "values, expected", [[[1.2, 4.5, 7.8], True], [[0.5, -1.2, 4.5], False]]
)
...
@pytest.mark.parametrize(
    "unclean_col_name, expected",
    [
        [
            [" col_name_1 "],
            ["col_name_1"],
        ],  # test case for leading and trailing whitespaces
        [["COL_NAME_2"], ["col_name_2"]],  # test case for uppercase
        [
            [" COL_NAME_3 "],
            ["col_name_3"],
        ],  # test case for leading and trailing whitespaces and uppercase
    ],
)
